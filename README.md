...
### 📊 분석 기능
- **채널 분석**: 특정 채널의 인기 영상(TOP 10) 및 개선 필요 영상(BOTTOM 10) 분석
- **키워드 분석**: 특정 키워드에 대한 트렌드 및 경쟁 영상 분석
- **라이징 스타**: 최근 급성장 중인 신규 채널 발굴
- **블루오션**: 경쟁이 적은 틈새 시장 분석
- **쇼츠 관리**: YouTube Shorts 성과 분석 및 개선 제안

### 🤖 AI 기능
- **AI 전략 생성**: Gemini 2.0을 활용한 맞춤형 성장 전략 제안
- **개별 영상 요약**: 영상별 AI 분석 및 개선 제안
- **쇼츠 최적화**: 쇼츠 성과 분석 및 AI 기반 개선 권장사항

### 📈 데이터 분석
- 인기도 점수 계산 (조회수 대비 좋아요/댓글 비율)
- 영상 길이별 필터링 (Shorts/Long 구분)
- 지역별 필터링 (한국/해외)
- CSV 다운로드 기능
...
│   ├── AnalysisChart.tsx     # 데이터 시각화 차트
│   └── ShortsManagement.tsx  # 쇼츠 관리 탭
├── services/
...
└── mcp/
    └── youtube_strategy_mcp.py  # MCP 서버 (선택사항)
```

## 🔧 최근 개선 사항

### 코드 품질 개선 (2025-01)
- ✅ **상수 관리**: 매직 넘버 제거 및 `constants.ts`로 중앙화
- ✅ **유틸리티 함수**: 중복 코드 제거 및 `utils.ts`로 모듈화
- ✅ **타입 안전성**: TypeScript strict 모드 및 타입 정의 강화
- ✅ **에러 처리**: 일관된 에러 메시지 및 상세한 디버깅 정보
- ✅ **성능 최적화**: 배치 처리, 캐싱 준비, 중복 API 호출 제거

### 보안 개선
- ✅ **환경변수 정리**: Vite 설정에서 중복 제거
- ✅ **보안 문서**: `SECURITY.md` 추가
- ✅ **.env.example**: API 키 템플릿 제공
- ⚠️ **주의**: API 키가 클라이언트 번들에 포함됨 (프로덕션에서는 백엔드 프록시 권장)
