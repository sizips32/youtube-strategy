...
class SearchChannelInput(BaseModel):
    ...
    api_key: Optional[str] = Field(
        default=None,
        description="YouTube Data API v3 key (optional if YOUTUBE_API_KEY env var is set)",
        min_length=20,
    )
...
def filter_videos_by_region(videos: List[VideoData], region: RegionFilter) -> List[VideoData]:
    """Filter videos by region based on title/description language (simple heuristic)"""
...
async def search_channel_by_name(api_key: str, channel_name: str) -> Optional[ChannelInfo]:
    ...
    return ChannelInfo(
        ...
        total_view_count=int(stats.get("viewCount", 0)) if stats.get("viewCount") else None,
...
async def youtube_search_channel(params: SearchChannelInput) -> str:
    ...
    Args:
        params: Input parameters containing:
            - api_key (optional): YouTube Data API v3 key (or use YOUTUBE_API_KEY env var)
            - channel_name: Channel name or handle to search
            - response_format: Response format - 'json' or 'markdown' (default: 'json')

    Returns:
        str: Channel information in JSON or Markdown format
    """
    try:
        api_key = resolve_api_key(params.api_key)
...
async def youtube_analyze_channel_videos(params: AnalyzeChannelVideosInput) -> str:
    ...
    Args:
        params: Input parameters containing:
            ...
            - view_type: 'TOP' for best videos or 'BOTTOM' for underperformers
            - max_results: Maximum videos to analyze (1-50, default: 20)
            - video_type_filter: Filter by 'ALL', 'SHORTS', or 'LONG'
...
async def youtube_search_keyword(params: SearchKeywordInput) -> str:
    ...
    Args:
        params: Input parameters containing:
            ...
            - region_filter: Filter by 'ALL', 'KOREA', or 'OVERSEAS'
            - video_type_filter: Filter by 'ALL', 'SHORTS', or 'LONG'
            - published_after: Optional date filter (ISO 8601 format)
            - response_format: Response format - 'json' or 'markdown'
...
async def youtube_find_rising_stars(params: FindRisingStarsInput) -> str:
    ...
    Args:
        params: Input parameters containing:
            ...
            - max_channels: Maximum channels to return (1-20, default: 10)
            - min_subscribers: Minimum subscriber count (default: 1000)
            - max_subscribers: Maximum subscriber count (default: 100000)
            - response_format: Response format - 'json' or 'markdown'
    ...
    try:
        ...
        channels = sorted(
            channels,
            key=lambda c: c.total_view_count / max(c.subscriber_count, 1) if c.total_view_count and c.subscriber_count else 0,
            reverse=True,
        )
...
async def youtube_find_blue_ocean_topics(params: FindBlueOceanTopicsInput) -> str:
    ...
    Args:
        params: Input parameters containing:
            - api_key (optional): YouTube Data API v3 key (or use YOUTUBE_API_KEY env var)
            - broad_category: Broad content category to analyze
            - max_topics: Maximum topics to return (1-20, default: 10)
            - response_format: Response format - 'json' or 'markdown'
